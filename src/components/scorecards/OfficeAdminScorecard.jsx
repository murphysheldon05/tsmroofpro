...
function PassFail({ value, onChange, disabled = false }) {
  return (
    <div className={`flex gap-2 ${disabled ? "pointer-events-none opacity-60" : ""}`}>
      <button type="button" onClick={() => onChange(true)} disabled={disabled} className={`px-3 py-1 rounded text-xs font-bold transition-all ${value === true ? "bg-emerald-500 text-white" : "bg-gray-100 text-gray-400 hover:bg-emerald-50"}`}>PASS</button>
      <button type="button" onClick={() => onChange(false)} disabled={disabled} className={`px-3 py-1 rounded text-xs font-bold transition-all ${value === false ? "bg-red-500 text-white" : "bg-gray-100 text-gray-400 hover:bg-red-50"}`}>FAIL</button>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-3xl mx-auto">
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This will unlock the submitted scorecard so an admin can make changes.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4">
          ...
          <textarea value={notes} disabled={isReadOnly} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Feedback, coaching notes, areas to improve..." className="w-full mt-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-400 resize-none disabled:opacity-60" />
        </div>
