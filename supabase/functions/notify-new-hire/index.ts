...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const accessListPlain = requiredAccess.length > 0
      ? requiredAccess.map(a => `- ${a}`).join("\n")
      : "- No specific access requested";
    ...
    for (const recipient of recipients) {
      ...
      const plainText = `New Hire Onboarding - Action Required

Hi ${recipient.recipient_name || 'HR Team'},

A new hire has been submitted and requires account setup.
